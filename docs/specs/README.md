...
| 24 | [public-deployment.md](public-deployment.md) | 2 | **Public-internet exposure** — supersedes auth.md's friends-and-family threat model. § 24.1 Cloudflare Tunnel ingress + trusted-proxy `CF-Connecting-IP` extraction (loopback-peer check); § 24.2 invite-code registration (`invites` table, `REGISTER` wire message, in-transaction redemption); § 24.3 IP-keyed sliding-window auth/register rate limiting; § 24.4 feedback-limit reinstate + connection cap; § 24.5 cloudflared + systemd runbook. **Draft 2026-06-02; not started.** | auth, persistence, wire codec, orchestrator, web client, ops. |
