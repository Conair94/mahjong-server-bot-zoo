...
| 22 | [layer8-closeout-r2.md](layer8-closeout-r2.md) | 1/2/3 | Round-2 follow-ups from live two-tab verify: § 22.1 claim-window arrow info leak (one-line renderer fix, supersedes Spec 21 §3); § 22.2 claim-action alerts (visual + future sound); § 22.3 selection highlight under Unicode (CSS); § 22.4 discard tile font size; § 22.5 **BUGANG-from-hand stalls hand loop** (Tier 1 engine bug — first priority); § 22.6 table-creation options + claim-resolution window (per-table bot speed + decide-timeout, 1.5 s minimum claim window for fair multi-human play); § 22.7 re-sort concealed; § 22.9 hand-end scoring summary (fan breakdown + point swing, renderer-only). | engine (gang DRAW), table manager, web client renderer, lobby UI. **ALL landed 2026-06-01: §22.1–22.5, §22.7, §22.9 + §22.6 Part A (table-creation options). §22.6 Part B found unnecessary (claim resolution already fair). Browser-verify pass owed for the UI items.** |
| 23 | [feedback-reporting.md](feedback-reporting.md) | 2/3 | In-game bug/feature reporting: `FEEDBACK` WS message (auth-implicit, post-auth phase-1), sanitisation allow-list as the security boundary, plain-text files in `var/reports/`, `<feedback-button>` Lit modal. **Landed 2026-06-01.** | orchestrator, wire codec, web client. |
| 24 | [public-deployment.md](public-deployment.md) | 2 | **Public-internet exposure** — supersedes auth.md's friends-and-family threat model. § 24.1 Cloudflare Tunnel ingress + trusted-proxy `CF-Connecting-IP` extraction (loopback-peer check); § 24.2 invite-code registration (`invites` table, `REGISTER` wire message, in-transaction redemption); § 24.3 IP-keyed sliding-window auth/register rate limiting; § 24.4 feedback-limit reinstate + connection cap; § 24.5 cloudflared + systemd runbook. **Implemented; merged via PR #1.** | auth, persistence, wire codec, orchestrator, web client, ops. |
| 25 | [admin-console.md](admin-console.md) | 2 | **Operator control console** — host-side GUI (control plane) supervising the `serve` data plane. New token-gated `GET /admin/status` on the server exposing the in-memory registry; `mahjong.control` package + `python -m mahjong control` CLI with a `ServerSupervisor` (subprocess start/stop/restart), psutil metrics, log ring buffer, `TunnelSupervisor` (cloudflared), and invite/account ops over the persistence layer; Lit/no-build admin web UI over one `mahjong-admin-v1` WS. Only new dep: `psutil`. **Draft 2026-06-03; not started.** | server config + lifecycle, orchestrator/registry (read), persistence, ops. |
| 26 | [scoring-config.md](scoring-config.md) | 1/2 | **Configurable scoring core** (Layer-9 / AI-plan build-order step 0). Makes the fan floor (`fan_cliff`) and the fan→points conversion (`conversion` block: `mcr-official` additive vs `house-table` tier-lookup) ruleset-config properties instead of engine constants; adds `mcr-house-3fan.json` (3-fan floor, convex house payout, renchan) + frozen `MANIFEST` hash; centralizes config-driven renchan (`next_dealer`). One scorer shared by live server + training reward (no skew). Corrects the AI-plan premise: the cliff was *already* enforced (hard-coded 8), so `mcr-2006` stays byte-identical and its goldens unchanged. False-mahjong penalty declared-but-deferred (unreachable in-engine). **Draft 2026-06-04; pre-implementation (TDD).** | engine (`pymj`, legality, `hu` transition, rulesets loader), orchestration (registry + web renchan), training reward (future). |
| 27 | [v0-offense-bot.md](v0-offense-bot.md) | 2 | **v0 offense bot** (Layer-9 / AI-plan build-order step 1). The first real decision seat: an in-process, offense-only, k=1 greedy bot (`mahjong/bots/v0.py` pure policy + `mahjong/adapters/v0.py` `V0Adapter`) replacing the `CannedAdapter`-PASS placeholder on `kind: "bot"` seats, making the server solo-playable. Core metric is **fan-aware distance** — shanten toward a *fan-feasible* archetype, not any 14-tile completion — so the bot never locks into a sub-floor (illegal) tenpai. Always HU when legal; claims PENG/CHI only on strict improvement; GANG/defense/opponent-modeling deferred to v1+. The fan-aware greedy baseline later architectures must beat. **Draft 2026-06-04; pre-implementation (TDD).** | server registry + web (bot-seat construction), table manager (consumes), `pymj` (shanten/fan), rulesets loader (floor). |
