...
- **Friends-and-family threat model.** No public-internet exposure ([server-plan.md § Hosting](../server-plan.md) puts the server on a Tailscale tailnet). The adversary we plan against is "someone got into the LAN and is trying to log in as Alice"; we are *not* planning against "an attacker has stolen the DB and is offline-cracking 10M passwords". That said, we use defensive hashing parameters anyway — they're cheap and the cost of getting them wrong is high if the threat model shifts.
