...
| DEF-16 | **Kong with an empty wall ends the hand without `HAND_END`** (found in the Spec 35 v1-vs-v0 eval): a bot drew the last wall tile and declared a concealed kong; with no replacement tile the engine went `TERMINAL` and the record closed (FOOTER present) **with no `HAND_END` event**. Live clients would wait on settlement forever; the eval aggregator skips the hand. Deterministic repro: `python scripts/eval_inprocess.py --bots v1,v0,v0,v0 --hands 58 --seed 20260611 --out <dir>` → `selfplay-00000057.jsonl`. Distinct from DEF-12 (that's a wedged task; this is a clean terminal missing an event). | Terminal-emission fix is core engine/manager work (kong replacement-draw path in [table/manager.py](../../mahjong/table/manager.py)) with its own TDD fixture; out of scope of the bot branch that surfaced it. | Fix on next engine touch, or immediately if a live hand ends with no settlement screen. | A record containing `"event":"FOOTER"` but no `"event":"HAND_END"`; repro seed above |
