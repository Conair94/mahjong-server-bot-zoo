...
| DEF-14 | **Flaky tests** (timing-sensitive reconnect family): `test_fixture_21a_disconnect_and_reconnect_within_hold_window` fails ~1-in-3 full-suite runs **on clean main** (reproduced 2026-06-11 while building FB-14; not introduced by it); `test_client_reconnects_and_reauths_after_drop` (tests/web/test_reconnect_e2e.py) failed once under full-suite load 2026-06-12 (FB-18 branch *and* its base both pass it in isolation — same socket-count-wait pattern). | Pre-existing; unrelated to the branches that surfaced them. | Next CI/local failure of either test, or any change to seat-hold timing in `sessions/mux.py`. | `pytest tests/server/test_multi_human_e2e.py::test_fixture_21a_disconnect_and_reconnect_within_hold_window`; `pytest tests/web/test_reconnect_e2e.py` |
...
- **Status:** **implemented** (2026-06-12). `handDisplayOrder` in prompt.js is
  now the single source of the on-screen hand order, shared by the renderer
  and the key layer (tile keys + arrows select by *display* position, stored
  as raw `origIdx`); Enter with no selection reads `view.last_drawn.tile`
  (no draw + no selection → no-op instead of discarding the sorted-last
  tile). Depends on FB-17's projection change (`last_drawn` must survive a
  refresh). **Verification:** `tests/web/test_drawn_tile_targeting.py` —
  three Playwright fixtures (bare-Enter tsumogiri, `]` targets the
  visually-last drawn tile, `9` targets the 9th displayed tile), all red
  before the fix; full web suite green.
