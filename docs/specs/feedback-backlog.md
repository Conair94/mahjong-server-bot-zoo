...
| DEF-23 | **Flaky test (auth race): `test_fixture_19_and_22_two_human_full_hand`** intermittently fails with `AUTH_RESPONSE {ok: False}` — first seen on the `ubuntu-latest / Python 3.13` CI cell 2026-06-12 (PR #44), passed on the other 3 cells and on re-run. A freshly `create_account`-ed user on a per-test `tmp_path` DB has its `AUTH_REQUEST` rejected, so the suspect is a race between the synchronous `create_account` write and the async (`run_in_executor`) auth verify, or argon2 timing under a loaded runner. Newly *observable* only because the CI pytest step finally runs (it died at `tests/web` collection before — DEF-22). Same multi-human-e2e flake family as DEF-14 (different test + symptom). | Pre-existing; non-deterministic (1-in-N, green on re-run), so no clean repro yet. Root-causing needs the race instrumented, not guessed. | Next CI/local failure of this test (grep the assert), or any change to the auth `run_in_executor` path / `create_account` write-visibility. | `assert resp["kind"] == "AUTH_RESPONSE" and resp.get("ok")` in [tests/server/test_multi_human_e2e.py:80](../../tests/server/test_multi_human_e2e.py); auth path in [server/registry.py](../../mahjong/server/registry.py) `handle_auth_request` |
