...
def test_win_loss_draw_counts(p: Persistence, account_id: int) -> None:
    ...
    _seed_hand(
        ...
        score_deltas={0: 24, 1: -8, 2: -8, 3: -8}, fan_total=8, started_at_ms=1000,
    )
    # Loss: account at seat 0, seat 1 wins.
    _seed_hand(
        p, account_id=account_id, seat=0, winner_seat=1,
        score_deltas={0: -8, 1: 24, 2: -8, 3: -8}, fan_total=8, started_at_ms=2000,
    )
    # Draw.
    _seed_hand(
        p, account_id=account_id, seat=0, winner_seat=None,
        score_deltas={0: 0, 1: 0, 2: 0, 3: 0}, fan_total=None, started_at_ms=3000,
...
def test_win_points_and_best_fan_only_count_wins(p: Persistence, account_id: int) -> None:
    ...
    _seed_hand(
        p, account_id=account_id, seat=0, winner_seat=0,
        score_deltas={0: 40, 1: -16, 2: -16, 3: -8}, fan_total=12, started_at_ms=1000,
    )
    # Win with fan 6.
    _seed_hand(
        p, account_id=account_id, seat=0, winner_seat=0,
        score_deltas={0: 20, 1: -8, 2: -8, 3: -4}, fan_total=6, started_at_ms=2000,
    )
    # Big loss — must not pollute total_win_points or best_win_fan.
    _seed_hand(
        p, account_id=account_id, seat=0, winner_seat=2,
        score_deltas={0: -32, 1: -8, 2: 48, 3: -8}, fan_total=24, started_at_ms=3000,
    )
    s = p.account_stats(account_id)
...
def test_in_progress_and_nonlive_excluded(p: Persistence, account_id: int) -> None:
    ...
    _seed_hand(
        p, account_id=account_id, seat=0, winner_seat=None,
        score_deltas={}, fan_total=None, started_at_ms=2000, finalize=False,
    )
    # Not counted: selfplay source.
    _seed_hand(
        p, account_id=account_id, seat=0, winner_seat=0,
        score_deltas={0: 24, 1: -8, 2: -8, 3: -8}, fan_total=8, started_at_ms=3000,
        source="selfplay",
    )
    s = p.account_stats(account_id)
...
def test_score_series_cumulative_ascending(p: Persistence, account_id: int) -> None:
    ...
    for ts, delta in ((1000, -24), (2000, 48), (3000, 10)):
        won = delta > 0
        _seed_hand(
            p, account_id=account_id, seat=0,
...
def test_score_series_window_cap(p: Persistence, account_id: int) -> None:
    ...
    for i in range(250):
        _seed_hand(
            p, account_id=account_id, seat=0, winner_seat=0,
            score_deltas={0: 1, 1: -1, 2: 0, 3: 0}, fan_total=8,
