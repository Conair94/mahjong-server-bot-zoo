...
def test_fixture1_loopback_peer_header_trust_on_returns_forwarded() -> None:
    assert (
        resolve_client_ip(
            peer_host="127.0.0.1", forwarded_for="203.0.113.7", trust_proxy=True
        )
        == "203.0.113.7"
    )
...
def test_fixture2_loopback_peer_header_trust_off_returns_peer() -> None:
    assert (
        resolve_client_ip(
            peer_host="127.0.0.1", forwarded_for="203.0.113.7", trust_proxy=False
        )
        == "127.0.0.1"
    )
...
def test_fixture3_nonloopback_peer_with_header_returns_peer_not_header() -> None:
    ...
    assert (
        resolve_client_ip(
            peer_host="203.0.113.9", forwarded_for="203.0.113.7", trust_proxy=True
        )
        == "203.0.113.9"
    )
...
def test_fixture4_loopback_peer_no_header_returns_peer() -> None:
    assert (
        resolve_client_ip(
            peer_host="127.0.0.1", forwarded_for=None, trust_proxy=True
        )
        == "127.0.0.1"
    )
...
def test_ipv6_loopback_peer_with_header_returns_forwarded() -> None:
    assert (
        resolve_client_ip(
            peer_host="::1", forwarded_for="203.0.113.7", trust_proxy=True
        )
        == "203.0.113.7"
    )
...
def test_missing_peer_host_falls_back_to_sentinel() -> None:
    assert (
        resolve_client_ip(peer_host=None, forwarded_for=None, trust_proxy=False)
        == "unknown"
    )
